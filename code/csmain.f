...
C                                                                      C
C  ADAPTED BY MARIE VAN DE SANDE(5) TO INCLUDE A THE EFFECT OF A       C
C  CLUMPY OUTFLOW AND STELLAR AND COMPANION PHOTONS                    C
C                                                                      C
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      ...
      CHARACTER*11 JAC
      CHARACTER*500 FRATES,FSPECS,FOUTF,FOUTN,INFILE,FILECO,FILEN2,
     *   FPP,FCD,OUTFOLDER,VELMODE,CLUMPMODE,TEMPMODE,FPPCD
...
C Data I/O unit numbers
      DATA USPEC,URATES,UFRAC,UNUM,UANA,NEXTRA,UIN/1,2,3,4,5,11,9/
      DATA UPP,UCD/7,8/
C Physical constants
      DATA PI,MH,KB/3.1415927,1.6605E-24,1.3807E-16/
C Step size in radius grid      
!      RESOLUTION = 3E-2
      
      
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC      
C                 FIRST STEP: READ EVERYTHING IN                       C
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC      
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCC
      ...
      DO I=1,2
         READ(UIN,*)
      END DO
      READ(UIN,*) DUMMY,DUMMY,FILECO
      READ(UIN,*) DUMMY,DUMMY,FILEN2
      READ(UIN,*)
...
c set ratio of integrated UV flux at Rscale = 50*Rstellar in IRC+10216 to integrated Draine ISM field
c integrated Draine UV field = 5.89e11 photons m-2 s-1 ster-1 over 912-2050 Angstroms
      GSTAR = GETGSTAR(T_STAR,RSCALE/R_STAR)
      write(*,*) 'GSTAR IN MAIN CODE = ', GSTAR
      
C SET RADIUS AT WHICH I CALCULATE PHOTORATES DUE TO BINARY PHOTONS
C TAKE THIS RADIUS TO BE THE SAME AS FOR STELLAR PHOTONS
C THIS ENABLES ONE TO DIRECTLY COMPARE PHOTORATES
...
CCC CLUMPING LOOP
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C     If a two-component model is requested, i.e. POROSITY or AGUNDEZ,
C     CLUMPING is set to 1 so that both components are calculated
...
C                 Everything is inside of clumps, only one component
...
C                 Nothing is in clumps, smooth case
      ...
      WRITE(*,*) 'Starting calculation of only interclump component...'
            ELSE
C                 Minor component of POROSITY or AGUNDEZ
                  OUTF='csfrac_clump'
                  OUTN='csnum_clump'
                  PP='csphyspar_clump'
                  CD='cscoldens_clump'
      ...
      ELSE
C           In case of SMOOTH, MILLAR: just one component
            OUTF='csfrac_smooth'
            OUTN='csnum_smooth'
            PP='csphyspar_smooth'
            CD='cscoldens_smooth'
...
C  PHYSICAL PARAMETER NAME STRINGS
      ...
      SPECI(NTOT+6) = 'C K(IP)'
      SPECI(NTOT+7) = 'C K(AP)'
      SPECI(NTOT+8) = 'CO K(PH)'
      SPECI(NTOT+9) = 'CO K(IP)'
      SPECI(NTOT+10) = 'CO K(AP)'
      SPECI(NTOT+11) = 'DELTA_AUV'
...
C  STEP NUMBER INCREMENT
      IRUN = IRUN + 1
!       write(*,*)  	
!       WRITE(*,256) L/FVOL*(ROUT/R_STAR)**(2/3.)   
!  256  FORMAT(1X,"H =  "1PE8.2)
...
c       write(*,*) IRUN
      ...
      B(NTOT+1,IRUN) = HNR
      B(NTOT+2,IRUN) = TEMP
c      B(NTOT+3,IRUN) = AUV, UV EXTINCTION BETWEEN R AND INFINITY
      B(NTOT+3,IRUN) = AUV
      B(NTOT+4,IRUN) = RAD
c      B(NTOT+5,IRUN) =  C ISM RATE
      B(NTOT+5,IRUN) = K(5827)
c      B(NTOT+6,IRUN) =  C IP RATE
      B(NTOT+6,IRUN) = K(6302)
c      B(NTOT+7,IRUN) =  C AP RATE
      B(NTOT+7,IRUN) = K(6638)
C    CO ISM RATE
      B(NTOT+8,IRUN) = K(ICOR)
C    CO IP RATE
      B(NTOT+9,IRUN) = K(ICOIP)
C    CO AP RATE
      B(NTOT+10,IRUN) = K(ICOAP)
C    UV EXTINCTION BETWEEN RDUST AND R
      B(NTOT+11,IRUN) = DELTA_AUV
...
!       WRITE(*,*) HNR,F(IH2,IRUN),F(IH2,IRUN)*2.
